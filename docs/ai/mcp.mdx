...
{
  "readsmith-docs": {
    "url": "https://readsmith.dev/docs/mcp"
  }
}
```

The friendly `/mcp` alias is a convenience on a standalone deploy. The canonical endpoint is `/_readsmith/mcp`, which answers on every host, so it is the reliable form to hand an agent. The quickest way to get the exact URL for your site is the **Copy MCP URL** action in any page's actions menu, which also offers one-click **Add to Cursor** and **Add to VS Code**. You can set a different alias with `mcp.path` in config.
