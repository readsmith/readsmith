...
[Environment](/self-host/environment) for the `STORAGE_*` variables). It wires the database with a health check, a persistent volume for each store, and the app port; you provide an `.env` created from `.env.example`.
